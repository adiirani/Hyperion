#include "../headers/PreProcessing.h"
    ...
    std::vector<std::vector<double>> vectorizers::tfidf(const std::vector<std::string>& input, const std::set<std::string>& stopWords) {
        ...
        for (const std::string& text : input) {
            std::vector<double> documentTermFrequencies(termIndex.size(), 0.0);
            std::vector<std::string> tokenized = tokenize(text, stopWords);
        ...
        std::vector<double> documentFrequencies(termIndex.size(), 0.0);
        ...
        for (const auto& docTermFreqs : termFrequencies) {
            ...
            for (int i = 0; i < docTermFreqs.size(); ++i) {
                ...
                double tf = docTermFreqs[i] / docTermFreqs.size();
    ...
    std::vector<std::vector<double>> vectorizers::hashing(const std::vector<std::string>& input, const std::set<std::string>& stopWords) {
        std::vector<std::vector<double>> result;
    ...
    std::unordered_map<double, std::vector<std::string>> vectorizers::getReverseDictionary() const {
