...
#include <sstream>
	...
	layer::layer(int nodeCt, ActivationFunction activationFunc) { //uses normal generation to randomly initialize weights and biases. TODO: switch to xavier glorot.
		std::random_device rd;
		std::mt19937 gen(rd());
		std::normal_distribution<double> dist(0.0, 1.0);
		ActivationFunction layerActivation = activationFunc;
		weights.resize(nodeCt, std::vector<double>(nodeCt));
		biases.resize(nodeCt);
		nodeCount = nodeCt;
		activFunction = activationFunc;

		//init random biases according to normal distribution.
		for (int y = 0; y < nodeCt; ++y) {
